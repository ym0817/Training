...
class ShuffleNetV2(nn.Module):
    ...
    def forward(self, x, is_feat=False, preact=False):
        ...
        out = F.avg_pool2d(out, (1,4))
        out = out.view(out.size(0), -1)    # 2048
        f4 = out
        out = self.linear(out)
        if is_feat:
            if preact:
...
if __name__ == '__main__':
    ...
    # feats, logit = net(x, is_feat=False, preact=False)
    b = time.time()
    print("time_cost: ", b - a)
    for f in feats:
    ...
    if 1:
        dummy_input = torch.randn(1, 1, 8, 32)
        torch.onnx.export(net,
                          dummy_input,
                          "empty_shufflenet_41.onnx",
                          opset_version=11,
                          verbose=False,
